...
def extract_content(php_file):
    if not os.path.exists(php_file):
        return "<p>Content being migrated...</p>"
    with open(php_file, 'r') as f:
        content = f.read()
    match = re.search(r'<div class="article-content[^>]*>(.*?)</div>\s*</article>', content, re.DOTALL)
    if not match:
        # Try finding in the new template structure
        match = re.search(r'<div id="introduction" class="article-content[^>]*>(.*?)</div>\s*<div class="mt-20', content, re.DOTALL)
    if match:
...
# Dynamic Index Page logic
blogs_json_string = json.dumps(blogs_data)
...
$filteredBlogs = array_filter($blogs, function($b) use ($currentCategory) {{
    ...
    // Exact match or contains (for cases like "Mental Health" vs "Health")
...
print("Blog logic fixed: PHP syntax error resolved.")
